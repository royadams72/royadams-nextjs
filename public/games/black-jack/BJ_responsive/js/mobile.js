var mobile = function() {
    //Global assets
    var mobileAssetsLoaded = false, m_fontObj = {}, m_stage, m_requestId, m_suit, m_stringArray, m_cardX, m_dealerCardX, m_playerCardX, m_msgBG, m_playerTxt, m_gameBG, m_dealerTxt, m_playerScore, m_msgtxt, m_dealerScore, m_bets, m_gameStarted, m_cardStartX, m_objScale, centerStage;
...
var initMobile = function() {
        ...
        if (!commonAssetsLoaded) { //if commonn assets not loaded create loader object, else, just add to loader  
            ...
            mloader.add('three_diamonds', clientlibUrl + 'img/cards/three_diamonds.png')
            ...
            mloader.add('seven_diamonds', clientlibUrl + 'img/cards/seven_diamonds.png')
            mloader.add('eight_diamonds', clientlibUrl + 'img/cards/eight_diamonds.png')
        ...
        mloader.load(function(loader, resources) {
                ...
                cardStringArray = ["two", "three", "four", "five", "six", "seven", "eight", "nine", "ten"];
                m_dealerCardArr = [];
                m_playerCardArr = [];
                m_cardsDealtArr = [];
                m_chipArray = [];
	...
	var shadow = shadowFilter();
	   ...
	   // m_msgContainer.filters = [shadow];
	    ...
	    m_objScale = .5;
	 	//Object for font
        m_fontObj = {
            ds: true, dsColor: '#262626', dsDistance: 3, fill: 0xffffff, align: 'center', clickFont: 'italic 50px Arial', tryAgainFont: 'italic 23px Arial', larger: 'italic 65px Arial', winTxtL: 'italic 40px Arial', winTxtS: 'italic 35px Arial', lineH: 45, ww: false, padding: 20
        };
    };	
...
var dealCard = function(player) {
    ...
    var getDealerCard = function(player) {
        ...
        var n = 0, a = 0, b = 0;
		var shadow = shadowFilter();
        if ( m_betCount < 2 ) { //first bet
			
            if (!m_standingClicked && m_betCount == 1) { //first 2 cards dealt
				n = getRandomNum(2, 5, false, false);
				//console.log(m_standCount);
            } else if ( m_standCount < 2 ) { //When stand button clicked, update m_standCount on each iteration
				//console.log(m_standCount);
        ...
        if (m_betCount == 2) { //Second bet
            if (!m_standingClicked && m_betCount == 2) {//First 2 cards dealt for second bet
       ...
       for (i = 0; i < m_dealerCardArr.length; i++) {
            if (cardName == m_dealerCardArr[i]) {
                count++;
                if (count == 2) {//Then duplicate found
        ...
        m_dealerScore += n;
        card = PIXI.Sprite.fromImage(clientlibUrl + "img/cards/" + cardName + ".png");
        ...
        if (!m_standingClicked && m_dealerCardArr.length == 2) {
           ...
           var t = TweenMax.to(cardContainer, 1.3, { x: m_dealerCardX + 70 });
            ...
            TweenMax.to(cardContainer, .9, { x: m_dealerCardX, onComplete: function() {
                    TweenMax.to(cardBack, .3, { width: 0, onComplete: function() {
                            TweenMax.set(cardBack, { alpha: 0 })
                            TweenMax.set(card, { alpha: 1 })
                            TweenMax.to(card, .3, { width: card.endWidth });
                            updateScore("dealer");
                        }
                    });
                }
 ...
 var getPlayerCard = function(player) {
	...
	// console.log(n)
        ...
        TweenMax.to(cardContainer, .9, { x: m_playerCardX, onComplete: function() {
                updateScore("player");
                TweenMax.to(cardBack, .3, { width: 0, onComplete: function() {
                        TweenMax.set(cardBack, { alpha: 0 })
                        TweenMax.set(card, { alpha: 1 })
                ...
                if (m_playerCardArr.length == 2) {
					...
					TweenMax.delayedCall(0.3, function () { btnAnim(); btnActive(stand, false); btnActive(hit, false);});
					m_gameStarted = true;
                }
            }
        })
...
var playersDisplay = function() {
    var starY = 40;
        for (i = 0; i < 2; i++) {
            ...
            if (i == 0) { 
                m_dealerTxt = new PIXI.Text("Dealer: " + m_dealerScore, { font: 'bold 19px Arial', fill: 0x000000, align: 'center' });
                m_dealerTxt.anchor.set(0.5, 0.5);
                m_dealerTxt.x = 40//star.width / 2 - 5;
                m_dealerTxt.y = 70;
                amountContainer.addChild(m_dealerTxt);
            } else {
                m_playerTxt = new PIXI.Text("You: " + m_playerScore, { font: 'bold 18px Arial', fill: 0x000000, align: 'center' });
 ...
 var buildWinMsg = function() {
        ...
        var topPadding = 40;
        var centerNum;
        var firstTxt = new PIXI.Text(gameSettings.youWon + '\n' + gameSettings.signUp + '\n', {
            font: f.winTxtS,
            dropShadow: f.ds,
 ...
 var firstDeal = function() {
		TweenMax.to(m_gameContainer, 1, {alpha: 1, onComplete: function (){
		dealCard("dealer");
        dealCard("player");
 ...
 var addListeners = function() {
    ...
    var resetCards = function() {
        ...
        for (var i = 0; i < m_cardsDealtArr.length; i++) {
            ...
            //
            TweenMax.to(card, duration, { y: -200, onComplete: function() {
...
var resetVars = function() {
 ...
 var standCards = function() {
    ...
    var flipDealerCards = function(c) {
		...
		var card = cardContainer.getChildAt(0);
		 TweenMax.to(cardContainer, .3, {x: cardContainer.endX - 17});
        TweenMax.to(cardContainer.getChildAt(1), .3, { width: 0, onComplete: function() {
 ...
 var checkScore = function(player) {
	// console.log(m_betCount);
        if (player == "player") {
            ...
            if (m_dealerScore == 21) {
                ...
                stopTweens(m_standArr);
                playerWins();
            }else if ( m_betCount == 1 && m_dealerScore > m_playerScore && m_dealerScore >= 17 ) {
                stopTweens(m_standArr);
                dealerWins("blackjack");
            }
        }
    ...
    var playerWins = function() {
        ...
        TweenMax.delayedCall(2, function() {
			msgTween("in");
            TweenMax.to(m_gameContainer, .3, { alpha: 0 });
 ...
 var dealerWins = function (e) {
        ...
        m_msgtxt.style = { font: f.clickFont, fill: f.fill, align: f.align, lineHeight: f.lineH, wordWrap: f.ww, padding: f.padding};
		 if (e == "busted"){
			  m_msgtxt.text = gameSettings.badLuck + " \n" + gameSettings.youBusted + " \n" + gameSettings.click + " \n" + gameSettings.AnotherBet;
		 }else{
			  m_msgtxt.text = gameSettings.badLuck + " \n" + gameSettings.dealerWins + " \n" + gameSettings.click + " \n" + gameSettings.AnotherBet; 
		 }
    
        if (m_bets == 1) { //Then m_chipArray[1] is inActive so...
            pulse(m_chipArray[0]);
        } else { //Then m_chipArray[0] is inActive so...
            pulse(m_chipArray[1]);
        }
...
var msgTween = function (e){
	if(e == "in"){
		TweenMax.delayedCall(.8, function (){ TweenMax.to(m_msgContainer, .8, {x: centerStage}); })
	
	}else{
		TweenMax.to(m_msgContainer, .9, {x: 680, onComplete: firstDeal}) ;
    ...
    var addText = function () {
        ...
        var topPadding = 0;
        var centerNum;
        m_msgtxt = new PIXI.Text(gameSettings.click + '\n' + gameSettings.bet + '\n' + gameSettings.getStarted, {
            font: f.clickFont,
            fill: f.fill,
            align: 'center'
        });
