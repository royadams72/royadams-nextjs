...
var deskTop = function () {
  ...
  var gameSettings = {
    clickThroughURL: "",
    badLuck: "Bad Luck.",
    tryAgainTxt1: "Try again",
    tryAgainTxt2: "One last try",
    clickSpin: "Click 'Spin'",
    toPlay: "to play",
    youWon: "You’ve won!",
    winLine1: "Sign up now to receive",
    winLine2: "50 FREE SPINS",
    winLine3: "+ UP TO £400",
    winLine4: "DEPOSIT BONUS",
  };
  var initDeskTop = function () {
    ...
    mloader
      .load(function (loader, resources) {
        ...
        renderer = new PIXI.autoDetectRenderer(900, 500, {
          transparent: true,
          view: game,
        });
  ...
  var editSprites = function () {
    ...
    fontObj = {
      ...
      fill: 0xe4e44a,
      align: "center",
      clickFont: "32px Arial",
      tryAgainFont: "23px Arial",
      small: "30px Arial",
      larger: "35px Arial",
    };
  };
  ...
  var addBtn = function () {
    ...
    btn.startY = btn.y;
    btn.startX = btn.x;
    btnGlow.scale.y = btnGlow.scale.x = 0.5;
    //console.log(this.btnGlow)
    btnGlow.alpha = 0;
    btn.addChild(btnGlow);
    spinBtn.scale.y = spinBtn.scale.x = 0.5;
  ...
  var youWonAnim = function () {
	...
	*/
    ...
    TweenMax.to(winContainer, 2, {
      width: 330,
      height: 70,
      onComplete: function () {
        TweenMax.to(winContainer, 0.6, { alpha: 0, onComplete: winFrame });
      },
    });
  };
  ...
  var sparkles = function () {
    for (var i = 0; i < 50; i++) {
      ...
      TweenMax.to(sparkle, sparkle.animTime, {
        width: sparkle.num,
        height: sparkle.num,
      });
  ...
  var winMessage = function () {
    ...
    var topPadding = 0;
    var centerNum;
    var firstTxt = new PIXI.Text(gameSettings.winLine1, {
      font: f.larger,
      fill: f.fill,
      align: f.align,
    });
    var SecondTxt = new PIXI.Text(gameSettings.winLine2, {
      font: f.larger,
      fill: f.fill,
      align: f.align,
    });
    var thirdTxt = new PIXI.Text(gameSettings.winLine3, {
      font: f.larger,
      fill: f.fill,
      align: f.align,
    });
    var fourthTxt = new PIXI.Text(gameSettings.winLine4, {
      font: f.larger,
      fill: f.fill,
      align: f.align,
    });
    var joinGlow = new PIXI.Sprite(mloader.resources.joinGlow.texture);
    var youWonTxt = new PIXI.Text(gameSettings.youWon, {
      font: f.larger,
      fill: f.fill,
      align: f.align,
    });
  ...
  var setMessages = function () {
    var f = fontObj;
    tryAgain1 = new PIXI.Text(
      gameSettings.badLuck + " " + gameSettings.tryAgainTxt1,
      { font: f.clickFont, fill: f.fill, align: f.align }
    );
    tryAgain2 = new PIXI.Text(
      gameSettings.badLuck + " " + gameSettings.tryAgainTxt2,
      { font: f.clickFont, fill: f.fill, align: f.align }
    );
    click = new PIXI.Text(gameSettings.clickSpin + " " + gameSettings.toPlay, {
      font: f.clickFont,
      fill: f.fill,
      align: f.align,
    });
  ...
  var btnPulse = function (b) {
    ...
    t = TweenMax.to(bGlow, 0.2, { alpha: 1 });
    tweenArray.push(t);
    t1 = TweenMax.to(b, 0.5, {
      x: "-=15",
      y: "-=15",
        ...
        TweenMax.to(b, 0.3, {
          ...
          onComplete: function () {
            if (!btnClicked) {
              t.pause(0);
              t1.pause(0);
              t2 = TweenMax.delayedCall(0.5, function () {
                t1.play();
                t.play();
              });
              tweenArray.push(t2);
  ...
  var buildBlur = function () {
    ...
    for (var i = 0; i < SLOT_NUMBER; i++) {
      ...
      blur =
        blurSpriteArray[Math.floor(Math.random() * blurSpriteArray.length)];
      ...
      for (var j = 0; j < 2; j++) {
        ...
        blurSprite.scale.y = blurSprite.scale.x = 0.5;
  ...
  var createSlots = function () {
    ...
    for (var i = 0; i < SLOT_NUMBER; i++) {
      ...
      for (var j = 0; j < 3; j++) {
        ...
        slot.width = 200;
        slot.height = 200;
        slot.scale.y = slot.scale.x = 0.5;
        endY = j * (slot.height / 2);
        slot.y = endY - 30;
        slot.endY = endY;
        slot.filters = [blurFilter1];
        blurFilter1.blur = 10;
        TweenMax.to(slot, delay, {
          y: slot.endY,
          ease: Bounce.easeOut,
          onComplete: function () {
            blurFilter1.blur = 0;
    ...
    if (spins != 3) {
      ...
      if (!firstTime) {
        ...
        var t = TweenMax.delayedCall(1, function () {
          if (spins == 1) {
            t2 = TweenMax.to(tryAgain1, 0.5, {
              alpha: 1,
              onComplete: addListeners,
            });
          }
          if (spins == 2) {
            t2 = TweenMax.to(tryAgain2, 0.5, {
              alpha: 1,
              onComplete: addListeners,
            });
          }
      ...
      TweenMax.delayedCall(1.5, function () {
        TweenMax.to(reelContainer, 1, { alpha: 0 });
      });
