...
var deskTop = function() {
    ...
    var fontObj = {},
        ...
        stage, requestId, suit, stringArray, cardX, dealerCardX, playerCardX, msgBG, playerTxt, dealerTxt, playerScore, msgtxt, dealerScore, bets, gameStarted, cardStartX, updateCardX;
...
var getDealerCard = function(player) {
        ...
        if (betCount < 2) { //first bet
            if (!standingClicked && betCount == 0) { //first 2 cards dealt
				n = getRandomNum(2, 5, false, false);
				firstCardNum++;
            } else if ( standCount < 2 ) { //When stand button clicked, update standCount on each iteration
        ...
        if (betCount == 2) { //Second bet
            if (!standingClicked && betCount == 2) {//First 2 cards dealt for second bet
        ...
        if (!standingClicked && dealerCardArr.length == 2) {
           ...
           var t = TweenMax.to(cardContainer, 1.3, {x: dealerCardX + 90
            ...
            TweenMax.to(cardContainer, .9, { x: dealerCardX, onComplete: function() {
				//console.log(dealerCardX)
                    TweenMax.to(cardBack, .3, { width: 0, onComplete: function() {
                            ...
                            TweenMax.to(card, .3, { width: cardEndWidth });
                            updateScore("dealer");
                        }
                    });
                }
 ...
 var getPlayerCard = function(player) {
        ...
        gameContainer.addChild(cardContainer);
        cardsDealtArr.push(cardContainer);
        TweenMax.to(cardContainer, .9, { x: playerCardX, onComplete: function() {
                updateScore("player");
                TweenMax.to(cardBack, .3, { width: 0, onComplete: function() {
                        TweenMax.set(cardBack, { alpha: 0 })
                        TweenMax.set(card, { alpha: 1 })
                ...
                if (playerCardArr.length == 2) {
                    TweenMax.delayedCall(.3, startGame);
                    gameStarted = true;
                }
            }
        })
...
var playersDisplay = function() {
    ...
    var firstDeal = function() {
        TweenMax.delayedCall(.32, dealCard, ["dealer"]);
        dealCard("player");
...
var resetVars = function() {
 ...
 var standCards = function() {
    ...
    var flipDealerCards = function(c) {
		...
		console.log(cardContainer.width)
        TweenMax.to(cardContainer.getChildAt(1), .3, { width: 0,onComplete: function() {
                ...
                TweenMax.to(cardContainer.getChildAt(0), .3, { width: 107 });
            }
        });
 ...
 var checkScore = function(player) {
        if (player == "player") {
            ...
            if (dealerScore == 21) {
                ...
                stopTweens(standArr);
                playerWins();
            }else if (betCount == 1 && dealerScore > playerScore && dealerScore >= 17) {
                stopTweens(standArr);
                dealerWins();
            }
        }
    ...
    var dealerWins = function () {
        ...
        if (bets == 1) { //Then chipArray[1] is inActive so...
            pulse(chipArray[0]);
        } else { //Then chipArray[0] is inActive so...
            pulse(chipArray[1]);
        }
    ...
    var addText = function () {
        ...
        var topPadding = 0;
        var centerNum;
        msgtxt = new PIXI.Text(gameSettings.click + ' ' + gameSettings.bet + ' ' + gameSettings.getStarted, {
            font: f.small,
            fill: f.fill,
            align: f.align
        });
