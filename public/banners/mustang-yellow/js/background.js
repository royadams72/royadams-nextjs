	...
	var tw1;
	var mTxtStartPos = "top:24px; left: 81px; width: 156px; height: 35px; opacity: 0;";
	...
	var spawner = function () {
		if (playBG) {
			if (spawnTimer == 0) {
				calculateXY();
				var explColour = colourPaleteArr[Math.floor(Math.random() * colourPaleteArr.length)];
	...
	function Particle() {
		...
		this.draw = function (mainContext2D) {
	...
	function update(frameDelay) {
		if (playBG) {
			...
			mainContext2D.fillRect(0, 0, mainContext2D.canvas.width, mainContext2D.canvas.height);
	...
	var initBG = function () {
		...
		tw1 = TweenLite.to(carModelContainer, 2, {
			alpha: 1,
			onComplete: function(){TweenLite.delayedCall(1.5, startParticles)}
		})
	...
	var startParticles = function () {
		TweenLite.to(carModel_a, .4, {alpha:0})
		TweenLite.to(carModelContainer, .7, {
		width: cName.cWidth,
			top: cName.topPos,
			left:cName.leftPos,
			ease: Power2.easeOut
